...
//   Semester     -> folder-name detection ONLY (never guessed by AI —
//                   it's a required field, a wrong guess is worse than
//                   skipping the file for manual review)
...
async function processJob(job, ctx) {
  ...
  if (!semester || Number.isNaN(semesterNumber)) {
    semesterNumber = 3; // Default Fallback to Semester 3
  }
...
try {
  ...
  const subject = ai?.subject || subjectHint || fallbackSubject;
  ...
  if (DRY_RUN) {
    ...
    console.log(`📝 [dry run] "${item.name}" -> "${title}" | ${subject} | Sem ${semesterNumber} | ${category}`);
    return;
  }
...
uploadResult = await withRetry(() => uploadBufferToCloudinary(buffer, { extension: currentExtension }), {
  ...
  report.imported.push({ file: item.name, title, subject, category, semester: semesterNumber, aiUsed: Boolean(ai) });
  console.log(`✅ Imported "${title}"  [Sem ${semesterNumber} | ${subject} | ${category}]${ai ? "" : "  (heuristic fallback)"}`);
}
